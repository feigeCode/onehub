...
impl ClickHouseDatabaseForm {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let engine_select = cx.new(|cx| {
            SelectState::new(engine_items, Some(IndexPath::new(0)), window, cx)
        });

        let comment_input = cx.new(|cx| {
            InputState::new(window, cx)
                .placeholder("数据库注释 (可选)")
        });
        ...
        let engine_sub = cx.subscribe_in(&engine_select, window, |this, _select, _event: &SelectEvent<Vec<EngineSelectItem>>, _window, cx| {
            this.trigger_form_changed(cx);
        });
    ...
    fn build_request(&self, cx: &App) -> DatabaseOperationRequest {
        ...
        let engine = self.engine_select.read(cx)
            .selected_value()
            .cloned()
