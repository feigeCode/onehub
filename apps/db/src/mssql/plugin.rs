...
impl DatabasePlugin for MsSqlPlugin {
    ...
    fn rename_table(&self, _database: &str, old_name: &str, new_name: &str) -> String {
        format!("EXEC sp_rename '{}', '{}'", old_name.replace("'", "''"), new_name.replace("'", "''"))
    }
    ...
    fn build_column_def(&self, col: &ColumnDefinition) -> String {
        ...
        if let Some(len) = col.length {
            ...
            } else if col.data_type.to_uppercase() == "NVARCHAR" || col.data_type.to_uppercase() == "VARCHAR" {
...
mod tests {
    ...
    fn test_build_create_database_sql() {
        ...
        field_values.insert("collation".to_string(), "SQL_Latin1_General_CP1_CI_AS".to_string());
    ...
    fn test_build_alter_table_sql_add_column() {
        ...
        let new = TableDesign {
            ...
            columns: vec![
                ColumnDefinition::new("id").data_type("INT"),
                ColumnDefinition::new("email").data_type("NVARCHAR").length(100),
            ],
            indexes: vec![],
            foreign_keys: vec![],
            options: TableOptions::default(),
    ...
    fn test_build_alter_table_sql_drop_column() {
        ...
        let original = TableDesign {
            ...
            columns: vec![
                ColumnDefinition::new("id").data_type("INT"),
                ColumnDefinition::new("old_column").data_type("NVARCHAR").length(50),
            ],
            indexes: vec![],
            foreign_keys: vec![],
            options: TableOptions::default(),
