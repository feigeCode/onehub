...
impl ContextInferrer {
    ...
    fn determine_context(tokens: &[&SqlToken], paren_depth: i32) -> SqlContext {
                ...
                for i in (0..paren_idx).rev() {
                    match &tokens[i].kind {
                        SqlTokenKind::Keyword(SqlKeyword::Table) => {
                            ...
                            if i > 0 {
                                if let SqlTokenKind::Keyword(SqlKeyword::Create) = &tokens[i - 1].kind {
                                    return SqlContext::CreateTable;
                                }
                            }
                            break;
                        }
                        SqlTokenKind::Whitespace | SqlTokenKind::Ident | SqlTokenKind::QuotedIdent => {
                            // Continue searching
                        }
                        _ => {
        ...
        match last_keyword_info {
            Some((idx, keyword)) => {
                Self::context_from_keyword(keyword, tokens, idx)
            }
    ...
    fn context_from_keyword(keyword: &SqlKeyword, tokens: &[&SqlToken], keyword_idx: usize) -> SqlContext {
        ...
        match keyword {
            ...
            SqlKeyword::From | SqlKeyword::Join | SqlKeyword::Inner | SqlKeyword::Left
            | SqlKeyword::Right | SqlKeyword::Full | SqlKeyword::Cross => {
                // Check if we've moved past the table name
                // After table name, we might have alias, ON, WHERE, etc.
                if Self::has_keyword_after(tokens_after_keyword, &[
                    SqlKeyword::Where, SqlKeyword::On, SqlKeyword::Group,
                    SqlKeyword::Order, SqlKeyword::Having, SqlKeyword::Limit,
                ]) {
                    return Self::find_later_context(tokens_after_keyword);
                }
                ...
                let ident_count = tokens_after_keyword
                    .iter()
                    .take_while(|t| {
                        matches!(t.kind, SqlTokenKind::Ident | SqlTokenKind::QuotedIdent
                            | SqlTokenKind::Dot | SqlTokenKind::Comma)
            ...
            SqlKeyword::Where | SqlKeyword::And | SqlKeyword::Or | SqlKeyword::On
            | SqlKeyword::Having => {
    ...
    fn find_later_context(tokens: &[&SqlToken]) -> SqlContext {
        ...
        for token in tokens.iter().rev() {
            if let SqlTokenKind::Keyword(kw) = &token.kind {
                match kw {
                    SqlKeyword::Where | SqlKeyword::And | SqlKeyword::Or
                    | SqlKeyword::On | SqlKeyword::Having => {
                    ...
                    SqlKeyword::From | SqlKeyword::Join | SqlKeyword::Inner
                    | SqlKeyword::Left | SqlKeyword::Right | SqlKeyword::Full
                    | SqlKeyword::Cross => {
                        return SqlContext::TableName;
                    }
                    SqlKeyword::Select => {
...
mod tests {
    ...
    fn test_select_columns_context() {
        ...
        assert_eq!(infer_context("SELECT DISTINCT ", 16), SqlContext::SelectColumns);
    }
    ...
    fn test_table_name_context() {
        ...
        // After JOIN
        assert_eq!(infer_context("SELECT * FROM users JOIN ", 25), SqlContext::TableName);
        // After LEFT JOIN
        assert_eq!(infer_context("SELECT * FROM users LEFT JOIN ", 30), SqlContext::TableName);
    }

    #[test]
    fn test_condition_context() {
        // After WHERE
        assert_eq!(infer_context("SELECT * FROM users WHERE ", 26), SqlContext::Condition);
        // After AND
        assert_eq!(infer_context("SELECT * FROM users WHERE id = 1 AND ", 37), SqlContext::Condition);
        // After OR
        assert_eq!(infer_context("SELECT * FROM users WHERE id = 1 OR ", 36), SqlContext::Condition);
    }

    #[test]
    fn test_order_by_context() {
        assert_eq!(infer_context("SELECT * FROM users ORDER BY ", 29), SqlContext::OrderBy);
        assert_eq!(infer_context("SELECT * FROM users GROUP BY ", 29), SqlContext::OrderBy);
    }

    #[test]
    fn test_dot_column_context() {
        // After alias.
        assert_eq!(infer_context("SELECT u.", 9), SqlContext::DotColumn("u".to_string()));
        assert_eq!(infer_context("SELECT u.id, u.", 15), SqlContext::DotColumn("u".to_string()));
        // With table name
        assert_eq!(infer_context("SELECT users.", 13), SqlContext::DotColumn("users".to_string()));
    }
    ...
    fn test_function_args_context() {
        ...
        assert_eq!(infer_context("SELECT MAX(id, ", 15), SqlContext::FunctionArgs);
    }

    #[test]
    fn test_set_clause_context() {
        assert_eq!(infer_context("UPDATE users SET ", 17), SqlContext::SetClause);
    }

    #[test]
    fn test_values_context() {
        assert_eq!(infer_context("INSERT INTO users VALUES ", 25), SqlContext::Values);
    }
    ...
    fn test_create_table_context() {
        ...
        assert_eq!(infer_context("CREATE TABLE users (", 20), SqlContext::CreateTable);
        assert_eq!(infer_context("CREATE TABLE users (id ", 23), SqlContext::CreateTable);
        assert_eq!(infer_context("CREATE TABLE users (id INT, name ", 33), SqlContext::CreateTable);
    }
    ...
    fn test_complex_query_context() {
        ...
        assert_eq!(infer_context(sql, offset), SqlContext::DotColumn("users".to_string()));
    }
    ...
    fn test_dot_column_with_join_alias() {
        ...
        assert_eq!(infer_context(sql, offset), SqlContext::DotColumn("departments".to_string()));
    }
    ...
    fn test_dot_column_unknown_alias() {
        ...
        assert_eq!(infer_context(sql, offset), SqlContext::DotColumn("x".to_string()));
    }
