...
impl SymbolTable {
    ...
    pub fn register_alias(&mut self, alias: &str, table: &str) {
        ...
        self.aliases.insert(key, (table.to_string(), self.current_scope));
    }
    ...
    pub fn exit_scope(&mut self) {
        if self.current_scope > 0 {
            self.aliases.retain(|_, (_, scope)| *scope < self.current_scope);
            self.current_scope -= 1;
        }
    }
    ...
    pub fn all_aliases(&self) -> impl Iterator<Item = (&str, &str)> {
        self.aliases.iter().map(|(k, (v, _))| (k.as_str(), v.as_str()))
    }
...
impl SymbolTable {
    ...
    fn parse_table_references(
        ...
        while i < tokens.len() {
            ...
            if matches!(tokens[i].kind, SqlTokenKind::LParen) {
                ...
                if i < tokens.len() {
                    ...
                    if i < tokens.len() && matches!(tokens[i].kind, SqlTokenKind::Ident) {
                        // Subquery alias - we don't have a table name, skip
                        i += 1;
                    }
                }
                continue;
            ...
            if !matches!(tokens[i].kind, SqlTokenKind::Ident | SqlTokenKind::QuotedIdent) {
                break;
            }
